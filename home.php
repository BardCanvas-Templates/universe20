...
<head>
    ...
    <!-- This template -->
    <link rel="stylesheet" type="text/css" href="<?= $template->url ?>/media/styles~v<?= "{$template->version}-3" ?>.css">
    ...
    <? if( $template->count_right_sidebar_items() > 0 ): ?>
        <!-- Right sidebar -->
        <link rel="stylesheet" type="text/css" href="<?= $template->url ?>/media/right_sidebar_addon~v<?= "{$template->version}-1" ?>.css">
    <? endif; ?>
...
<div id="body_wrapper">
    ...
    <div id="header">
        ...
        <?
        if( $settings->get("templates:universe20.hide_slider") != "true" &&
            $settings->get("modules:posts.slider_categories") != "" &&
            empty($_GET["offset"]) )
            include __DIR__ . "/segments/home_slider.inc";
        ?>
    ...
    $lsclass = $template->count_left_sidebar_groups() > 0 ? "with_left_sidebar"  : "no_left_sidebar";
    $rsclass = $template->count_right_sidebar_items() > 0 ? "with_right_sidebar" : "no_right_sidebar";
    ?>
    
    <?
    $sections = $settings->get("templates:universe20.pre_content_sections");
    if( ! empty($sections) )
        foreach(explode("\n", $sections) as $section)
            include __DIR__ . "/home_sections/" . trim($section) . ".inc";
    ?>
    ...
    <div id="content_wrapper" class="clearfix">
        ...
        <div id="content" class="<?= $lsclass ?> <?= $rsclass ?>">
            <?
    ...
    <?
    $sections = $settings->get("templates:universe20.post_content_sections");
    if( ! empty($sections) )
        foreach(explode("\n", $sections) as $section)
            include __DIR__ . "/home_sections/" . trim($section) . ".inc";
    ?>
